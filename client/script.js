...
const chagneToBlack = (event) => {
    sayHelloButton.style.backgroundColor = "black";
    sayHelloButton.style.color = "white";
}

sayHelloButton.addEventListener('mouseover', chagneToBlack)
...
const chagneBack = (event) => {
    sayHelloButton.style.backgroundColor = "#EFEFEF";
    sayHelloButton.style.color = "black";
}

sayHelloButton.addEventListener('mouseout', chagneBack)
...
helloText.addEventListener('mousemove', sayHello);
...
const ohMy = () => {
    ...
    .then(res => {
        ...
        for (let loopingNumber = 0; loopingNumber < res.data.length; loopingNumber++) {
            let pElement = document.createElement("p");
            pElement.textContent = res.data[loopingNumber];
            htmlBody.appendChild(pElement);
...
/*
const callback = (anotherPram) => {
    console.log(anotherPram)
}
...
const repeatMyParam = () => {
    ...
    axios.get(`http://locationhost:3000/repeat/${somePram}`)
    .then(res => {
        callback(res.data);
    })
    .catch(theseHands => console.log(theseHands))
...
const repeatMyParam = () => {
    ...
    axios.get(`http://localhost:3000/repeat/${somePram}`)
    .then(res => {
        console.log(res.data)
...
const crazyQuery = () => {
    ...
    let crazy = "pizza&?=pie" //Are you crazy?
    axios.get(`http://localhost:3000/query-test/?query=${crazy}`)
    .then(res => {
        console.log(res.data)
    })
    .catch(theseHands => console.log(theseHands))
...
document.getElementById("query-button").addEventListener('click', crazyQuery)
...
// 1) nothing
// 2) it sends both queryies to the server
